...
|-----------|------|---------|
| **Nothing configured yet (no GPU, no API keys)** | — | Offer 3 paths: (1) FAL.ai free tier, (2) prompt-only, (3) Comfy Cloud |
...
**IMPORTANT — "Nothing configured yet?"** If this machine has no GPU AND no
API keys are set up for any cloud service (Comfy Cloud, FAL.ai, etc.), the
user has three honest options:

| Option | What it means | Best when |
|--------|--------------|-----------|
| **① FAL.ai free tier** | Register at fal.ai → get API key → add to `.env` as `FAL_API_KEY`. Use `--backend fal` or set `IMAGE_GEN_BACKEND=fal` in `.env`. Immediate, no GPU needed. | Quickest path to live generation |
| **② Prompt-only** | Agent generates a polished text-to-image prompt; user pastes it into Midjourney / DALL·E / Adobe Firefly / etc. | User prefers their own account |
| **③ Comfy Cloud** | Install comfy-cli + subscribe → get API key → use `--host https://cloud.comfy.org` | User wants the full ComfyUI ecosystem |

Always present the options and let the user choose. Do not silently proceed
down a path that requires a credential the user hasn't supplied.
