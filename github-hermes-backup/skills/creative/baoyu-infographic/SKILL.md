...
| Insurance Proposal | `linear-progression` + `morandi-journal` | Use for: HK insurance long-term value comparison (premium paid vs. surrender value at key ages), 3-phase withdrawal timeline, client profile summary |
| Sales Deck Cover | `bento-grid` + `corporate-memphis` | Use for: Client-facing one-page proposal summary with key metrics |
