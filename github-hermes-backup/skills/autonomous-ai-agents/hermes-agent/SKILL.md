...
For OpenClaw → Hermes migration notes (private GitHub repo discovery, script paths, merge strategy for MEMORY.md), see:
- `references/openclaw-migration-notes.md`
...
## System Snapshot & Upgrade Safety

Hermes stores all user data OUTSIDE the main program directory. Upgrading the package does NOT touch user data.

| Data | Location | Safe on upgrade? |
|------|----------|-----------------|
| Memory | `~/.hermes/MEMORY.md`, `~/.hermes/memories/` | ✅ |
| Config | `~/.hermes/config.yaml`, `~/.hermes/.env` | ✅ |
| Skills | `~/.hermes/skills/` | ✅ |
| Cron jobs | `~/.hermes/cron/jobs.json` | ✅ |
| Sessions | `~/.hermes/sessions/` (SQLite) | ✅ |
| Scripts | `~/.hermes/cron/scripts/` | ✅ |

**⚠️ PITFALL: Backup script gaps.** The default `hermes_backup.sh` may not cover all critical files. Before any upgrade, verify the backup includes at minimum:
- `MEMORY.md`, `SOUL.md`, `AGENTS.md`, `USER.md`, `IDENTITY.md`
- `config.yaml`, `.env`
- `skills/` (entire directory)
- `cron/jobs.json`

If any are missing, update the backup script BEFORE upgrading.

**Upgrade workflow:**
1. Run the backup script (`~/.hermes/cron/scripts/hermes_backup.sh`)
2. Check GitHub backup repo confirms push succeeded
3. Run `hermes update`
4. Verify config and memory are intact
5. If something broke → restore from GitHub backup, report version in MEMORY.md

**Version tracking:** After any upgrade, record in MEMORY.md:
```
| YYYY-MM-DD | hermes-agent X.Y.Z | upgrade: what changed |
```

This makes rollback straightforward — you always know what version you were on before.
...
## Compression Tuning Reference

MiniMax-M2.7 上下文压缩调参文档：`references/compression-tuning.md`
